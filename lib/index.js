...
const addMultiplier = (cost, currentQuantity, multipler) => Math.floor(cost * multipler ** currentQuantity);
const calculateMultiplerCost = (cost, currentQuantity, multipler, purchaseQuantity) => {
...
class Farm {
    ...
    buy(item, buyQuantity = 1) {
        if (item in ProducerType) {
            ...
            const multiplerCost = calculateMultiplerCost(producerInfo.cost, currentQuantity, producerInfo.multiplier, buyQuantity);
            ...
            const multiplerCost = calculateMultiplerCost(sellerInfo.cost, currentQuantity, sellerInfo.multiplier, buyQuantity);
            if (this.farmBank <= multiplerCost) {
                return message('Not enough money', false);
            }
            this.farmBank -= multiplerCost;
